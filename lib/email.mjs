    ...
    <tr><td align="center">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;background:#FFFFFF;border:1px solid #ECECE7;border-radius:4px;">
        <tr><td style="padding:28px 32px 18px 32px;border-bottom:3px solid #C97B19;background:#1F3D2C;border-radius:4px 4px 0 0;">
          ...
          <div style="font-family:Georgia,serif;font-size:12px;color:#E0913B;letter-spacing:0.08em;text-transform:uppercase;margin-top:4px;">The A-List Celebrity System™</div>
        </td></tr>
...
export function customerConfirmationEmail({ booking, businessName, businessPhone, confirmUrl, meetUrl }) {
  const dateLong = formatLongDate(booking.slot.date);
  const time = formatTime12h(booking.slot.time);
  const tz = escapeHtml(booking.displayTimezone || booking.slot.tz || 'America/New_York');
  const subject = `Your ${businessName} strategy call is booked — ${dateLong} at ${time}`;
  const meet = typeof meetUrl === 'string' && meetUrl ? meetUrl : null;
  const intro = meet
    ? `Your free strategy call is set for <strong>${dateLong} at ${time}</strong> (${tz}). We'll meet on Google Meet — your link is below.`
    : `Your free strategy call is set for <strong>${dateLong} at ${time}</strong> (${tz}). I'll call you then.`;
  ...
  const html = WRAPPER_OPEN + `
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 24px 0;border:1px solid #ECECE7;border-radius:4px;">
      ...
      <tr><td style="padding:14px 18px;">
        ...
        <div style="margin-bottom:8px;"><strong>Where:</strong> ${meet ? 'Google Meet (video)' : 'Phone call — I\'ll call you'}</div>
        <div><strong>Reference:</strong> ${booking.id}</div>
      </td></tr>
    </table>
    ${meet ? `<p style="text-align:center;margin:0 0 8px 0;">
      <a href="${escapeHtml(meet)}" style="display:inline-block;background:#1F3D2C;color:#FFFFFF;padding:15px 34px;text-decoration:none;font-family:Georgia,serif;font-weight:700;letter-spacing:0.04em;font-size:15px;border-radius:9999px;">Join the Google Meet call →</a>
    </p>
    <p style="text-align:center;margin:0 0 24px 0;font-size:13px;color:#6B6B6B;word-break:break-all;">or paste this link: <a href="${escapeHtml(meet)}" style="color:#C97B19;">${escapeHtml(meet)}</a></p>` : ''}
    ...
    <ul style="margin:0 0 24px 0;padding-left:20px;">
      ...
      <li style="margin-bottom:6px;">Exactly how the A-List Celebrity System™ would work for your business.</li>
    ...
    <p style="margin:24px 0 0 0;font-size:14px;color:#6B6B6B;">A calendar invitation is attached${meet ? ' (the Meet link is on the event)' : ''}. Need to reschedule? Just reply to this email.</p>
    ` + WRAPPER_CLOSE;
  ...
  const text =
    ...
    (meet ? `Join on Google Meet: ${meet}\n` : `Where: phone call — I'll call you\n`) +
    `\n` +
...
export function ownerNotificationEmail({ booking, meetUrl }) {
  const dateLong = formatLongDate(booking.slot.date);
  const time = formatTime12h(booking.slot.time);
  ...
  const html = WRAPPER_OPEN + `
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 16px 0;border:1px solid #ECECE7;font-size:14px;">
      ...
      <tr><td style="padding:8px 12px;background:#F4EFE6;font-weight:700;">Meet</td><td style="padding:8px 12px;">${meet ? `<a href="${escapeHtml(meet)}" style="color:#C97B19;">${escapeHtml(meet)}</a>` : 'Phone call (no Meet link generated)'}</td></tr>
...
// Notification to Kareem from the /grant-access page. Two kinds:
//  - 'added': client added Kareem as a Manager — he should accept the Google invite.
//  - 'help':  client can't grant access (likely doesn't control the profile) — detective info.
export function gbpAccessRequestEmail({ kind, name, email, business, listing, details }) {
  const isHelp = kind === 'help';
  const who = business || name || email || 'A client';

  const subject = isHelp
    ? `[GBP HELP] ${who} — can't grant access`
    : `[GBP] ${who} added you as a manager — accept the invite`;
  ...
  const rowData = [
    ...
    ['Email', email],
    ['Listing', listing],
    isHelp ? ['Who set it up / details', details] : null,
  ].filter(Boolean).filter(([, v]) => v && String(v).trim());

  const rows = rowData.map(([k, v]) => {
    let val;
    if (k === 'Email') val = `<a href="mailto:${escapeHtml(v)}" style="color:#C97B19;">${escapeHtml(v)}</a>`;
    else if (k === 'Listing') val = `<a href="${escapeHtml(v)}" style="color:#C97B19;word-break:break-all;">${escapeHtml(v)}</a>`;
    else val = escapeHtml(String(v)).replace(/\n/g, '<br>');
    return `<tr><td style="padding:8px 12px;background:#F4EFE6;font-weight:700;width:34%;vertical-align:top;">${k}</td><td style="padding:8px 12px;">${val}</td></tr>`;
  }).join('');

  const intro = isHelp
    ? `This client can't add you themselves — they likely don't control the profile yet. Use the details below to track down who owns it.`
    : `${who} says they've added you as a Manager. Watch for the Google Business Profile invite and click <strong>Accept</strong> — invites sometimes land in spam.`;

  const cta = isHelp ? '' : `
    <p style="text-align:center;margin:0 0 8px 0;">
      <a href="https://business.google.com" style="display:inline-block;background:#1F3D2C;color:#FFFFFF;padding:14px 30px;text-decoration:none;font-family:Georgia,serif;font-weight:700;letter-spacing:0.04em;font-size:15px;border-radius:9999px;">Open Business Profile Manager →</a>
    </p>`;

  const html = WRAPPER_OPEN + `
    <h1 style="font-family:Georgia,serif;font-size:22px;font-weight:600;color:#1F3D2C;margin:0 0 16px 0;">${isHelp ? 'Access help needed' : 'New manager invite incoming'}</h1>
    <p style="margin:0 0 20px 0;">${intro}</p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 20px 0;border:1px solid #ECECE7;font-size:14px;">
      ${rows || '<tr><td style="padding:8px 12px;color:#6B6B6B;">No details provided.</td></tr>'}
    </table>
    ${cta}
    <p style="margin:20px 0 0 0;font-size:14px;color:#6B6B6B;">Reply to this email to reach the client directly.</p>
    ` + WRAPPER_CLOSE;

  const text =
    (isHelp ? 'Access help needed' : 'New manager invite incoming') + '\n\n' +
    intro.replace(/<[^>]+>/g, '') + '\n\n' +
    rowData.map(([k, v]) => `${k}: ${v}`).join('\n') + '\n';
