...
import { createMeetingForBooking } from './gcal.mjs';
...
export async function handleCreateBooking(req, res) {
  ...
  // Slot is now locked. Create the Google Meet event and persist its link; this
  // degrades gracefully (meetUrl stays null) if Google isn't configured or errors.
  const businessName = process.env.BUSINESS_NAME || 'Hikmon';
  const meeting = await createMeetingForBooking(result.booking, cfg, { businessName });
  if (meeting?.meetUrl) {
    result.booking.meetUrl = meeting.meetUrl;
    result.booking.calendarEventId = meeting.eventId;
    result.booking.meetingProvider = meeting.provider;
    await updateBookingMeeting(result.booking.id, {
      meetUrl: meeting.meetUrl,
      calendarEventId: meeting.eventId,
      provider: meeting.provider,
    }).catch((e) => log.warn(`could not persist meeting for ${result.booking.id}: ${e?.message || e}`));
  }
  ...
  return sendJson(res, 200, ok({
    ...
    meetUrl: result.booking.meetUrl || null,
  }));
}
...
async function fireEmails(booking, cfg) {
  ...
  const ics = buildICS({
    ...
    description:
      `Your free strategy call with ${businessName}. Reference: ${booking.id}.` +
      (meetUrl ? `\nJoin on Google Meet: ${meetUrl}` : '') +
      `\n${confirmUrl}`,
    location: meetUrl || 'Phone call',
  });

  const customer = customerConfirmationEmail({ booking, businessName, confirmUrl, meetUrl });
  const owner = ownerNotificationEmail({ booking, meetUrl });
...
export async function handleGetBookingIcs(req, res, id) {
  ...
  const ics = buildICS({
    ...
    description:
      `Your free strategy call with ${businessName}. Reference: ${b.id}.` +
      (b.meetUrl ? `\nJoin on Google Meet: ${b.meetUrl}` : ''),
    location: b.meetUrl || 'Phone call',
  });
  res.writeHead(200, {
